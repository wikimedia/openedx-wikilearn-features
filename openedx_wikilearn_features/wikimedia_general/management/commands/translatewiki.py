...
class Command(BaseCommand):
    ...
    def msgmerge(
        ...
        staged_files,
        base_lang="en",
        generate_po_file_if_not_exist=False,
        output_file_mapping: dict = {},
        exclude_files: list = [
            "wm-django.po",
            "wm-djangojs.po",
            "manual.po",
        ...
        Arguments:
            ...
            generate_po_file_if_not_exist: (bool) if True and destination path doesn't exist,
            it will create empty po file
    ...
    def update_translations_from_transifex(self, locales, staged_files, base_lang="en"):
        ...
        for lang in locales:
            ...
            for filename in staged_files:
                command = pomerge_command.format(from_path=f"{edx_dir}/{filename}", to_path=f"{wm_dir}/{filename}")
                execute(command)

            self._move_files_from_src_to_dest(wm_dir, edx_dir, staged_files, delete_src_dir_if_empty=True)
    ...
    def generate_custom_strings(self, target_files, locales, base_lang="en", prefix="wm"):
        ...
        self.msgmerge(
            locales,
            output_files,
            generate_po_file_if_not_exist=True,
            output_file_mapping=files_mapping,
            exclude_files=[],
        )
