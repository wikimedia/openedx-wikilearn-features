...
def _task_is_running(course_id, task_type, task_key):
    ...
    running_tasks = AdminReportTask.objects.filter(course_id=course_id, task_type=task_type, task_key=task_key)
...
def _update_instructor_task(admin_report_task, task_result):
    ...
    if admin_report_task.task_state == PROGRESS and len(admin_report_task.subtasks) > 0:
        ...
        log.info(
            "background task (%s), state %s:  result: %s",
            task_id,
            result_state,
            returned_result,
        )
...
def _handle_instructor_task_failure(admin_report_task, error):
    ...
    """
    log.info(
        "Admin Report task (%s) failed, result: %s",
        admin_report_task.task_id,
        str(error),
    )
