"""
Base class for Admin Report celery tasks.
"""
...
class BaseAdminReportTask(Task):  # lint-amnesty, pylint: disable=abstract-method
    """
    Base task class for use with AdminReportTask models.

    Permits updating information about task in corresponding AdminReportTask for monitoring purposes.

    Assumes that the entry_id of the Atasks_basedminReportTask model is the first argument to the task.

    The `entry_id` is the primary key for the AdminReportTask entry representing the task.  This class
    ...
    def on_success(self, task_progress, task_id, args, kwargs):  # lint-amnesty, pylint: disable=arguments-differ
        """
        Update AdminReportTask object corresponding to this task with info about success.
        ...
        This is JSON-serialized and stored in the task_output column of the AdminReportTask entry.

        """
        TASK_LOG.debug("Task %s: success returned with progress: %s", task_id, task_progress)
        # We should be able to find the AdminReportTask object to update
        ...
        entry_id = args[0]
        entry = AdminReportTask.objects.get(pk=entry_id)
        ...
        if len(entry.subtasks) == 0:
            entry.task_output = AdminReportTask.create_output_for_success(task_progress)
            entry.task_state = SUCCESS
            entry.save_now()

    def on_failure(self, exc, task_id, args, kwargs, einfo):
        """
        Update AdminReportTask object corresponding to this task with info about failure.
        ...
        The information is recorded in the AdminReportTask object as a JSON-serialized dict
        ...
        try:
            ...
            # if the AdminReportTask object does not exist, then there's no point
            # trying to update it.
            TASK_LOG.error("Task (%s) has no AdminReportTask object for id %s", task_id, entry_id)
        else:
            TASK_LOG.warning("Task (%s) failed", task_id, exc_info=True)
            entry.task_output = AdminReportTask.create_output_for_failure(einfo.exception, einfo.traceback)
