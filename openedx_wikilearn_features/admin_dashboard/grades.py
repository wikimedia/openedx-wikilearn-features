...
from xmodule.split_test_block import get_split_user_partitions

from openedx_wikilearn_features.admin_dashboard.runner import TaskProgress
from openedx_wikilearn_features.admin_dashboard.utils import (
    upload_multiple_course_csv_to_report_store,
)
...
class GradeReportBase:
    ...
    def _batch_users(self, context):
        ...
        def get_enrolled_learners_for_course(course_id, verified_only=False):
            ...
            """
            self.log_additional_info_for_testing(context, "ProblemGradeReport: Starting batching of enrolled students")
            ...
            for user_ids in user_chunks:
                ...
                users = (
                    get_user_model()
                    .objects.filter(id__gte=min_id, id__lte=max_id, **filter_kwargs)
...
class MultipleCourseGradeReport:
    ...
    def generate(
        ...
        with modulestore().bulk_operations(course_key):
            ...
            return MultipleCourseGradeReport()._generate(
                context, courses
    ...
    def _success_headers(self):
        ...
        """
        return ["Course", "Total Student", "Average Grade"]
    ...
    def _upload(self, context, success_headers, success_rows, error_headers, error_rows):
        ...
        if len(error_rows) > 0:
            ...
            upload_multiple_course_csv_to_report_store(
                error_rows, "multiple_courses_grade_report_err", context.course_id, date
            )
    ...
    def _rows_for_users(self, context, users, courses):
        ...
        for course in courses:
            ...
            context = _CourseGradeReportContext(
                context.task_id,
                context.entry_id,
                course_key,
                context.task_input,
            ...
            success_rows.append([context.course.display_name, total_students, average_course_grade])
        return success_rows, error_rows
...
class CourseProgressReport:
    ...
    def generate(
        ...
        with modulestore().bulk_operations(course_key):
            context = _CourseGradeReportContext(task_id, _entry_id, course_key, _task_input, action_name)
            return CourseProgressReport()._generate(
    ...
    def _success_headers(self, course_id, user_id):
        ...
        return ["Student ID", "Email", "Username"] + self._unit_header(course_id, user_id)
    ...
    def _user_unit_progress(self, user, course_id_string, user_id):
        ...
        course_blocks = get_course_outline_block_tree(request, course_id_string, user)
        if "children" in course_blocks:
            for section in course_blocks["children"]:
                if "children" in section:
                    for sub_section in section["children"]:
    ...
    def _rows_for_users(self, context, users, user_id):
        ...
        with modulestore().bulk_operations(context.course_id):
            ...
            for user in users:
                ...
                success_rows.append(
                    [user.id, user.email, user.username] + self._user_unit_progress(user, course_id_string, user_id)
                )
            return success_rows
