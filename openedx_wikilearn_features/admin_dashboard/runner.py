...
from openedx_wikilearn_features.admin_dashboard.models import PROGRESS, AdminReportTask
...
def run_main_task(entry_id, task_fcn, action_name, user_id):
    ...
    Arguments passed to `task_fcn` are:
     ...
     `entry_id` : the primary key for the AdminReportTask entry representing the task.
     `course_id` : the id for the course.
     `task_input` : dict containing task-specific arguments, JSON-decoded from AdminReportTask's task_input.
    ...
    # Get the AdminReportTask to be updated. If this fails then let the exception return to Celery.
    ...
    fmt = "Task: {task_id}, AdminReportTask ID: {entry_id}, Course: {course_id}, Input: {task_input}"
    ...
    # Check that the task_id submitted in the AdminReportTask matches the current task
    # that is running.
    ...
    TASK_LOG.info(
        "%s, Task type: %s, Finishing task: %s",
        task_info_string,
        action_name,
        task_progress,
    )
